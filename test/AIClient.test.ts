...
describe("AIClient", () => {
  ...
  afterEach(() => {
    ...
    if ((global.fetch as unknown as { mockRestore?: () => void }).mockRestore) {
  ...
  test("should use config values if provided", () => {
    ...
    expect(c.baseUrl).toBe(config.baseUrl);
    expect(c.apiKey).toBe(config.apiKey);
    expect(c.modelName).toBe(config.modelName);
  });
  ...
  test("should fall back to env values if config is missing", () => {
    ...
    const client = new AIClient();
    const c = client as unknown as Record<string, unknown>;
    expect(c.baseUrl).toBe("https://env-url.com/v1");
    expect(c.apiKey).toBe("env-key");
    expect(c.modelName).toBe("env-model");
  });
  ...
  test("chatCompletion should throw if baseUrl is missing", async () => {
    ...
    try {
      ...
      expect((e as Error).message).toBe("AI_API_BASE_URL is not configured properly or is invalid.");
    }
  });
